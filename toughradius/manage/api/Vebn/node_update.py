...
@permit.route(r"/api/Vebn/node/update")
class NodeAddHandler(ApiHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if not node:
                return self.render_verify_err(u'the node for node_id : %s is not exist')

            node_name = request.get('node_name')

            if node_name:
                item = dataform.Item("node_name", rules.len_of(2, 32), description=u"区域名称")
                if item.validate(node_name):
                    node.node_name = node_name
                else:
                    return self.render_verify_err("node_name is not satistied")
            #if form.d.node_desc:
