...
node_add_vform = dataform.Form(
    dataform.Item("id", rules.not_null, description=u"区域id"),
...
@permit.route(r"/api/Vebn/node/add")
class NodeAddHandler(ApiHandler):
    ...
    def post(self):
        ...
        try:
            ...
            self.db.commit()
            return self.render_success(msg=u'区域添加成功:%s' % utils.safeunicode(form.d.node_name))
        except Exception as err:
