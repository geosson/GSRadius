...
""" 客户新开户
(serviceId,userName,userAccount,userPassword,phone,address,number,area,userState,endTime,postageId,createTime)
values(54445,'11111','11111','hYNN61jF5WZoYyF440In+p6g8Aq38iQfumqvIbZ8j3EDS6MRo6uGn2ImEPUucdxu','13820752198',
'天下城五区2-2-8层8号底商二楼','111111111111111111111111111111',127,'1','2017-04-26',4,'2016-04-26 18:04:12');

"""

customer_add_vform = dataform.Form(
    dataform.Item("realname", description=u"用户姓名"),
    dataform.Item("node_id", rules.not_null, description=u"区域id"),
    dataform.Item("client_id", rules.not_null, description=u"客户id"),
...
@permit.route(r"/api/Vebn/customer/add")
class CustomerAddHandler(ApiHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if not self.db.query(models.TrNode).filter_by(id=form.d.node_id).count() > 0:
                return self.render_verify_err("the node is not exist")
            product = self.db.query(models.TrProduct).get(form.d.product_id)
            if not product:
                return self.render_verify_err("product is not exist")
        ...
        try:
            ...
            #    # 自由资费
            #    if int(form.d.free_bill_type or 9999) not in (FreeFeeDate,FreeFeeFlow, FreeTimeLen):
            ...
            account.client_id = form.d.client_id
